...
serve_homepage(Req, State) ->
    ...
    case erlbasic_accounts:find_by_username(Username) of
        {ok, {Project, Programmer, Name, StoredUsername}} ->
            ...
            HomeBasPath = filename:join(UserDir, "home.bas"),
            case file:read_file(HomeBasPath) of
                {ok, HomeBas} ->
...
render_home_from_bas(Username, Name, Project, Programmer, HomeBas) ->
    UsernameText = escape_html(binary_to_list(Username)),
    NameText = escape_html(binary_to_list(Name)),
...
default_homepage(Username, Name, Project, Programmer) ->
    UsernameText = escape_html(binary_to_list(Username)),
    NameText = escape_html(binary_to_list(Name)),
    PpnText = io_lib:format("[~w,~w]", [Project, Programmer]),
    iolist_to_binary([
        ...
        "<h1>", UsernameText, "</h1>",
        "<div class=\"sub\">", NameText, " • ", io_lib:format("~s", [PpnText]), "</div>",
